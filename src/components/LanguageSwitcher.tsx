  ...
  return (
    <div className="relative z-30" ref={switcherRef}>
      <motion.button
        ...
        whileTap={{ scale: 0.98 }}
      >
        <img src={currentLanguage.flagPath} alt={currentLanguage.name} className="w-5 h-5 rounded-md" />
          ...
          >
            {languages.map((language, index) => (
              <motion.button
                ...
                whileHover={{ x: 4 }}
              >
                <img src={language.flagPath} alt={language.name} className="w-5 h-5 rounded-md" />
                <span className="font-medium">{language.name}</span>
                {i18n.language === language.code && (
                  <motion.div
                    className="ml-auto w-2 h-2 bg-primary-600 rounded-md"
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
